...
本题的实现原理是：使用栈、队列或二分边界维护操作顺序，在每次操作后保持数据结构的题目约束。先读取标准输入并按题目格式拆分字段，使用代码中的`v`、`n`、`m`、`grid`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
flowchart TD
    ...
    C["n = v.shift.to_i"]
    D["until stack.empty?"]
    E["next if grid i  j  == '0'    seen  i, j  "]
    F["py_print # islands  # treasures  "]
    A --> B
    B --> C
    C --> D
    D --> E
...
```mermaid
flowchart TD
    S["寻宝图：题意状态"] --> I["v = py_input_data.split"]
    I --> P["n = v.shift.to_i"]
    P --> Q["until stack.empty?"]
    Q --> O["py_print # islands  # treasures  "]
    O --> F["结束"]
```
