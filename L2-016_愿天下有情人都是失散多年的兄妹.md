...
本题的实现原理是：使用栈、队列或二分边界维护操作顺序，在每次操作后保持数据结构的题目约束。先读取标准输入并按题目格式拆分字段，使用代码中的`v`、`n`、`people`、`ancestors`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
```mermaid
flowchart TD
    S["愿天下有情人都是失散多年的兄妹：题意状态"] --> I["v = py_tokens"]
    I --> P["n = v.shift.to_i"]
