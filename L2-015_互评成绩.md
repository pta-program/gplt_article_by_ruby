...
本题的实现原理是：先对记录按关键字排序，再按题目规定的优先级顺序处理，保证每一步选择满足当前最优条件。先读取标准输入并按题目格式拆分字段，使用代码中的`v`、`scores`、`row`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
flowchart TD
    ...
    C["n, k, m = v.shift 3 "]
    D["py_print scores.last m .map    x  format '%.3f', x ..."]
    A --> B
    B --> C
    C --> D
```
...
```mermaid
flowchart TD
    S["互评成绩：题意状态"] --> I["v = py_tokens.map &:to_i "]
    I --> P["n, k, m = v.shift 3 "]
    P --> Q["py_print scores.last m .map    x  format '%.3f', x ..."]
    Q --> O["py_print scores.last m .map    x  format '%.3f', x ..."]
    O --> F["结束"]
```
