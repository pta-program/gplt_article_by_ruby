...
本题的实现原理是：使用栈、队列或二分边界维护操作顺序，在每次操作后保持数据结构的题目约束。先读取标准输入并按题目格式拆分字段，使用代码中的`lines`、`n`、`rows`、`v`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
names =
  rows
    .sort_by { |unique, ratio, name| [-unique, ratio, name] }
    .first(3)
    .map(&:last)
...
```mermaid
flowchart TD
    S["点赞狂魔：题意状态"] --> I["lines = py_lines"]
    I --> P["n = lines.shift.to_i"]
    P --> Q["py_print names.join ' '  "]
    Q --> O["py_print names.join ' '  "]
    O --> F["结束"]
```
