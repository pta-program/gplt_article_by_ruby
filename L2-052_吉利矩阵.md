...
本题的实现原理是：建立状态数组，按照题目给出的转移关系逐项更新，最后从目标状态恢复答案。先读取标准输入并按题目格式拆分字段，使用代码中的`rows`、`build`、`dp`、`next_dp`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
flowchart TD
    A["吉利矩阵：入口"]
    B["limit, size = py_tokens.map &:to_i "]
    C["rows =   "]
    D["dp.each do  current, ways "]
    E["if prefix.length == size - 1"]
    F["py_print dp.fetch Array.new size, limit , 0 .to_s "]
    A --> B
    B --> C
    C --> D
    D --> E
...
```mermaid
flowchart TD
    S["吉利矩阵：题意状态"] --> I["limit, size = py_tokens.map &:to_i "]
    I --> P["rows =   "]
    P --> Q["dp.each do  current, ways "]
    Q --> O["py_print dp.fetch Array.new size, limit , 0 .to_s "]
    O --> F["结束"]
```
