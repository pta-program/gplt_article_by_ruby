...
本题的实现原理是：按题目格式读取字符串或数值，逐项维护必要状态，并在每次判断时直接执行题目给出的规则。先读取标准输入并按题目格式拆分字段，使用代码中的`it`、`days`、`answers`、`started`保存计算过程，直接完成计算；处理完成后按照题目规定的顺序和格式输出结果。
...
```mermaid
flowchart TD
    S["阅览室：题意状态"] --> I["it = py_iter py_tokens "]
    I --> P["days = py_next it .to_i"]
    P --> Q["if action == 'S'"]
